...
def process_audio(y, sr, num_seconds):
  ...
  for i in range(num_seconds):
    ...
    r_norm, g_norm, b_norm = (r_avg / total, g_avg / total,
                              b_avg / total) if total != 0 else (0, 0, 0)
...
def process_audio_file(audio_file):
  ...
  mean_rgb, avg_rgb, least_frequent_rgb, top_12_rgbs = calculate_rgb_statistics(
    rgb_scores)

  rgb_scores_256 = normalize_rgb_256(rgb_scores)
  fft_data_256 = normalize_rgb_256(fft_data)

  save_data_to_csv("fft_data.csv",
                   ["Time (s)"] + [f"Frequency {i}" for i in range(768)],
                   [[i] + row.tolist() for i, row in enumerate(fft_data_256)])
  save_data_to_csv("rgb_scores.csv", ["Time (s)", "R", "G", "B"],
                   [[i] + row.tolist()
                    for i, row in enumerate(rgb_scores_256)])

  rgb_stats = np.array([mean_rgb, avg_rgb, least_frequent_rgb] +
                       top_12_rgbs.tolist())
  rgb_stats_256 = normalize_rgb_256(rgb_stats)
  labels = ["Mean", "Average", "Least Frequent"
            ] + [f"Top {i+1}" for i in range(12)]
  save_data_to_csv("rgb_stats.csv", ["Metric", "R", "G", "B"],
                   [[label] + row.tolist()
                    for label, row in zip(labels, rgb_stats_256)])


def process_audio_file(wav_file):
  # existing code to convert wav to rgb omitted for brevity

  # run scores_visualizer_data.py to create rgb_scores_time.csv
  subprocess.run([
    "python", "scores_visualizer_data.py", "rgb_scores.csv",
    "rgb_scores_time.csv"
  ])

  # return path to rgb_scores_time.csv
  return "rgb_scores_time.csv"
